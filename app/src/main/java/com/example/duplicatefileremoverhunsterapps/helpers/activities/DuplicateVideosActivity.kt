...
import com.example.duplicatefileremoverhunsterapps.mainadapters.VideosMainAdapter
...
import com.example.duplicatefileremoverhunsterapps.databinding.ActivityDuplicateVideosBinding
...
class DuplicateVideosActivity : AppCompatActivity(), MyAdapterListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchingDialog.setNameTitle("Scanning Videos")
    ...
    private fun getTotalVideoFiles() {
        val executor=Executors.newSingleThreadExecutor()
        executor.execute {
            val cursor: Cursor?
    ...
    private fun groupDuplicates() {
        ...
        executor.execute {
            ...
            filepath = cursor?.getColumnIndexOrThrow(MediaStore.Video.Media.DATA)!!
    ...
    private fun setAdapter(list: List<FilesModel>) {
        if(list.size>0){
            binding.recyclerView.adapter = VideosMainAdapter(list, this, this)
