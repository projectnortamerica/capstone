...
import com.example.duplicatefileremoverhunsterapps.mainadapters.AudioMainAdapter
...
import com.example.duplicatefileremoverhunsterapps.databinding.ActivityDuplicateAudiosBinding
...
class DuplicateAudiosActivity : AppCompatActivity(), MyAdapterListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchingDialog.setNameTitle("Scanning Audios")
    ...
    fun getTotalAudioFiles() {
        ...
        executor.execute {
            ...
            val projection = arrayOf(MediaStore.Audio.Media.DATA, MediaStore.Audio.Media._ID)
            cursor = applicationContext.contentResolver.query(uri, projection, null,
                null, null)
    ...
    private fun groupDuplicates() {
        ...
        executor.execute {
            ...
            filepath = cursor?.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA)!!
    ...
    private fun setAdapter(list: List<FilesModel>) {
        if (list.size > 0) {
            binding.recyclerView.adapter = AudioMainAdapter(list, this, this)
