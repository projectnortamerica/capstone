...
class OtherChildAdapter(val list:List<FilesChildModel>,
                        val context: Context, val adapterListener: MyAdapterListener
)
    : RecyclerView.Adapter<OtherChildAdapter.MyViewHolder>() {
   ...
   inner class MyViewHolder(val binding: ChildImageItemBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        Glide.with(context).load(R.drawable.btn_others).into(holder.binding.thumb)
