...
import com.example.duplicatefileremoverhunsterapps.databinding.ContactItemBinding

class ContactsChildAdapter(val list:List<FilesChildModel>,
                           val context: Context)
    : RecyclerView.Adapter<ContactsChildAdapter.MyViewHolder>() {
   ...
   inner class MyViewHolder(val binding: ContactItemBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(ContactItemBinding.inflate
            (LayoutInflater.from(parent.context),parent,false))
    }

    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        Glide.with(context).load(R.drawable.btn_contacts).into(holder.binding.thumb)
        ...
        holder.binding.tvNumber.text=list[position].path
        if (position==0){
